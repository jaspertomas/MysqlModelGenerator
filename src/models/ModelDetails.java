...
public class ModelDetails {
    //------------FIELDS-----------
    public static final String tablename=ModelDetail.tablename;
    public static String[] fields=ModelDetail.fields;
    public static String[] fieldtypes=ModelDetail.fieldtypes;
    //-----------------------
    //-------------------------TABLE FUNCTIONS---------------------

    //-----------getter functions----------
    /*
    public static ModelDetails getByName(String name)
    {
            HashMap<Integer,ModelDetails> map=select(" name = '"+name+"'");
            for(ModelDetails item:map)return item;
            return null;
    }	
    */
    public static ModelDetail getById(Integer id) {
            RecordList map=select(" id = '"+id.toString()+"'");
            for(ModelDetail item:map)return item;
            return null;
    }
    //-----------database functions--------------
    ...
    public static void delete(Integer id)
    {
        ...
        try { 
            ...
            Logger.getLogger(ModelDetails.class.getName()).log(Level.SEVERE, null, ex);
            ex.printStackTrace();
        }
    }
    public static void delete(ModelDetail item)
    {
        delete(item.getId());
    }
    public static Integer insert(ModelDetail item)
    {
        Connection conn=SqliteDbHelper.getInstance().getConnection();            
        Statement st = null;
    ...
    public static void update(ModelDetail item)
    {
        Connection conn=SqliteDbHelper.getInstance().getConnection();            
        Statement st = null;
        boolean withid=false;
    ...
    public static RecordList select(String conditions)
    {
        ...
        try { 
            ...
            while (rs.next()) {
                items.add(new ModelDetail(rs));
                    //items.put(rs.getInt("id"), new ModelDetails(rs));
            }
            return items;
        } catch (SQLException ex) {
            Logger.getLogger(ModelDetails.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public static String implodeValues(ModelDetail item,boolean withId)
    {
    ...
    public static String implodeFieldsWithValues(ModelDetail item,boolean withId)
    {
            ArrayList<String> values=item.implodeFieldValuesHelper(true);//get entire list of values; whether the id is included will be dealt with later.

            if(values.size()!=fields.length)
            {
                    System.err.println("ModelDetails:implodeFieldsWithValues(): ERROR: values length does not match fields length");
            }
    ...
    public static class RecordList extends ArrayList<ModelDetail>{
    ...
    public static void main(String args[])
    {
        try {
            deleteTable();
            createTable();
            //ModelDetail i=new ModelDetail();
            //i.save();
            
//            ModelDetails.delete(1);
            for(ModelDetail j:ModelDetails.select(""))
                System.out.println(j.getId());
            
            System.out.println(ModelDetails.count(""));
