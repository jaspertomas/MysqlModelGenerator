...
//	}
    ...
    public void delete()
    {
            Invoices.delete(this);
    }
    public void save()
    {
            if(id==null || id==0)
                    Invoices.insert(this);
            else
                    Invoices.update(this);
    }
    ...
    //-----------getter functions----------
    /*
    public static Invoices getByName(String name)
    {
            HashMap<Integer,Invoices> map=select(" name = '"+name+"'");
            for(Invoices item:map.values())return item;
            return null;
    }	
    */
    public static Invoices getById(Integer id) {
            HashMap<Integer,Invoices> map=select(" id = '"+id.toString()+"'");
            for(Invoices item:map.values())return item;
            return null;
    }
    //-----------database functions--------------
    ...
    public static void delete(Invoices item)
    {
        delete(item.getId());
    }
    public static void insert(Invoices item)
    {
        Connection conn=MySqlDBHelper.getInstance().getConnection();            
        Statement st = null;
    ...
    public static void update(Invoices item)
    {
        Connection conn=MySqlDBHelper.getInstance().getConnection();            
        Statement st = null;
        boolean withid=false;
    ...
    public static HashMap<Integer, Invoices> select(String conditions)
    {
        if(conditions.isEmpty())conditions = "1";
            ...
            try { 
                ...
                HashMap<Integer, Invoices> items=new HashMap<Integer, Invoices>();
                while (rs.next()) {
                    items.put(rs.getInt("id"), new Invoices(rs));
                }
                return items;
            } catch (SQLException ex) {
                Logger.getLogger(Invoices.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public static String implodeValues(Invoices item,boolean withId)
    {
    ...
    public static String implodeFieldsWithValues(Invoices item,boolean withId)
    {
            ArrayList<String> values=item.implodeFieldValuesHelper(true);//get entire list of values; whether the id is included will be dealt with later.

            if(values.size()!=fields.length)
            {
                    System.err.println("Invoices:implodeFieldsWithValues(): ERROR: values length does not match fields length");
            }
    ...
    public static void main(String args[])
    {
        ...
        HashMap<Integer,Invoices> items=Invoices.select("");
        for(Integer key:items.keySet())
        {
            Invoices item=items.get(key);
