...
public class Vehicles {
    //------------FIELDS-----------
    public static final String tablename=Vehicle.tablename;
    public static String[] fields=Vehicle.fields;
    public static String[] fieldtypes=Vehicle.fieldtypes;
    //-----------------------
    //-------------------------TABLE FUNCTIONS---------------------

    //-----------getter functions----------
    /*
    public static Vehicles getByName(String name)
    {
            HashMap<Integer,Vehicles> map=select(" name = '"+name+"'");
            for(Vehicles item:map)return item;
            return null;
    }	
    */
    public static Vehicle getById(Integer id) {
            RecordList map=select(" id = '"+id.toString()+"'");
            for(Vehicle item:map)return item;
            return null;
    }
    //-----------database functions--------------
    ...
    public static void delete(Vehicle item)
    {
        delete(item.getId());
    }
    public static void insert(Vehicle item)
    {
        Connection conn=SqliteDbHelper.getInstance().getConnection();            
        Statement st = null;
    ...
    public static void update(Vehicle item)
    {
        Connection conn=SqliteDbHelper.getInstance().getConnection();            
        Statement st = null;
        boolean withid=false;
    ...
    public static RecordList select(String conditions)
    {
        ...
        try { 
            ...
            while (rs.next()) {
                items.add(new Vehicle(rs));
                    //items.put(rs.getInt("id"), new Vehicles(rs));
            }
            return items;
        } catch (SQLException ex) {
            Logger.getLogger(Vehicles.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public static String implodeValues(Vehicle item,boolean withId)
    {
    ...
    public static String implodeFieldsWithValues(Vehicle item,boolean withId)
    {
            ArrayList<String> values=item.implodeFieldValuesHelper(true);//get entire list of values; whether the id is included will be dealt with later.

            if(values.size()!=fields.length)
            {
                    System.err.println("Vehicles:implodeFieldsWithValues(): ERROR: values length does not match fields length");
            }
    ...
    public static class RecordList extends ArrayList<Vehicle>{
